...
| Native tool catalog/dispatch | pinned `mcp-cpp` argument/filesystem/search/git/host/memory sources and AgenTTY `toolset_e2e_test.cpp` | filesystem/search/process/git suites plus `HostToolsTest`, `MemoryToolsTest`, `ToolRuntimeTest`, edit matcher/repair tests | shared alias/coercion layer, safe filesystem/search/map tools, bounded subprocesses, diagnostics, repository-aware git, todo/skill/docs/task host protocols, and remember/forget/wipe protocols | all catalog protocol shells except web translated; persistent memory/RAG/skill backends, idle-timeout/output-spill parity, richer grep context, cache parity, and native differential capture remain | partial |
