...
| Agent loop/reducer | `domain/session.hpp` phase/retry proofs plus doom-loop and salvage-dedup source suites | `SessionPhaseTest`, `ActiveTurnTest`, `DoomLoopBreakerTest`, `SalvagedCallDeduplicatorTest` | sealed four-phase typestate with active context absent from idle, exact 16-cell transition legality, context-preserving cancellation/retry/rate state, loop breaker, and immutable re-leak reducer | phase/retry invariants and helper suites translated; full message/effect reducer and headless loop remain | partial |
