...
| Agent loop/reducer | `domain/session.hpp` phase/retry proofs, `provider/error_class.hpp`, `tool/util/partial_json.cpp`, `runtime/app/update` stream/tool/compaction lifecycle, `cmd_factory.cpp` wire projection, scheduler, doom-loop, and salvage-dedup sources | `SessionPhaseTest`, `ActiveTurnTest`, `ProviderErrorPolicyTest`, `PartialJsonTest`, `ConversationWireTest`, `AgentReducerTest`, `AgentLoopTest`, `DispatcherToolPortTest`, `ToolSchedulerTest`, `DoomLoopBreakerTest`, `SalvagedCallDeduplicatorTest` | sealed four-phase typestate with active context absent from idle and exact transition legality; pure immutable submit/stream/tool/permission/cancel/compact/tick reducer with stale-turn rejection, UTF-8 8 MiB caps, typed effects, catalog-backed tool adapter, path-aware parallel scheduling, and a virtual-thread effect interpreter reusable by CLI/protocol/TUI/subagents; exact typed HTTP and wire-error classification, six-step jittered backoff ladders, Retry-After clamping, 90-second budget decay, monotonic four-attempt mid-stream protection, heartbeat/first-delta health resets, scheduled-retry deduplication/cancellation, and FIFO queued-turn draining; native 120-second streaming-silence watchdog with `Fresh`/`StallFired` deduplication, cancel-token trip, zero-delay synthetic transient error, and two-second slow-tick rebase guard, driven by an active-only 100 ms headless cadence; native partial-JSON closing and required-field guards, two-attempt same-context upstream-truncation recovery, retained block-close truncation signals, and non-retryable `max_tokens` protection; immutable wire-only compaction with latest-summary substitution, 65% compaction/95% normal soft ceilings, exact summary prompt, off-transcript bounded buffering, compaction-specific retry/cancel/queue behavior, persistence effects, post-turn auto-trigger, rapid-refill breaker, and quiet-turn re-enable; loop breaker and immutable re-leak reducer | foundational headless turns, tool continuations, transient/rate-limit/stall recovery, truncated tool-input recovery, and manual/automatic compaction execute end-to-end; auth refresh, executing-tool wedge watchdog, remaining top-level message domains, concrete composition-root persistence ownership, and canonical differential traces remain | partial |
