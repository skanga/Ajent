...
| RAG and repository map | pinned `mcp-cpp` repository map plus AgenTTY `rag_test.cpp`, `rag_rerank_test.cpp`, and advanced stemming/semantic/MMR/confidence/hot-reload/cache cases | `RepoMapToolsTest`, `RagCoreTest`, `RagRerankerTest`, `RagStemmerTest`, `RagCorpusFilesystemTest` | repository PageRank map; line-aligned semantic document chunks, fenced-code/list preservation, overlap-safe breadcrumb context, contextual BM25 with opt-in Porter vocabulary normalization, cosine, reciprocal-rank fusion, BM25 corpus and multi-query fusion; feature-fusion reranking, exact extractive compression, Jaccard MMR diversification, structured contexts and confidence; document-extension folder walks, hidden-directory pruning, v3 incremental cache, corruption fallback, and in-memory hot add/update/remove | deterministic lexical and filesystem-backed retrieval foundation ported; neural reranking, embeddings/HNSW, knowledge routing, memory fusion, and differential capture remain | partial |
