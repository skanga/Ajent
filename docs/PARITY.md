...
| Native tool catalog/dispatch | pinned `mcp-cpp` `arg_reader_test.cpp`, `fs_tools_test.cpp`, `search_tools_test.cpp`, tool sources, and AgenTTY `toolset_e2e_test.cpp` | `ArgReaderTest`, `WorkspaceSandboxTest`, `FileToolsTest`, `SearchToolsTest`, `RepoMapToolsTest`, `ToolRuntimeTest`, edit matcher/repair tests | shared alias/coercion layer, symlink-safe workspace/read-root boundary, typed result/errors/FileChange, durable filesystem dispatch, filename globbing, paginated regex grep, multi-language definition search, and budgeted personalized PageRank repository maps | pinned filesystem/search assertions translated; remaining process/git/web/host tools, richer grep context, cache parity, and native differential capture remain | partial |
