...
22-tool pinned catalog and source-correct 23-tool Ajent catalog, an actual
workspace write and structured change result,
...
Dedicated paired runs additionally activate a seeded skill, exercise the
complete memory mutation lifecycle, retrieve seeded BM25 documentation and a
remembered fact, and execute Ajent's source-defined PageRank map.
...
| Standalone `repo_map` publication | The pinned 0.2.8 MCP executable advertises 22 tools and rejects `repo_map` as unknown. The checked-in `serve.cpp` registers every native tool and the current `mcp_tools_bridge.cpp` registry includes `repo_map`. | Ajent's standalone MCP server publishes and executes the source-correct 23-tool recall-biased registry, including a real PageRank map. | `NativeMcpParityIT.skillsMemoryRagAndRepositoryMapExecutableBehaviorIsCharacterized` proves the native rejection, the two source statements, and Ajent's executable map over the paired fixture. Catalog comparisons remove only this declared source-ahead tool before comparing every common schema and annotation. |
...
| Rewind banner and recovery backup | AgenTTY restores a checkpoint destructively (overwrites edits, deletes files added since the checkpoint) and reports `rewound · files restored, prompt back in composer`, with no recovery path. | Before restoring, Ajent snapshots the current working tree to `refs/ajent/rewind-backups/<timestamp>` (newest 16 kept) so a rewind is recoverable, and the banner reads `rewound · files restored · backup at refs/ajent/rewind-backups`. | `GitCheckpointStoreTest.restoreBacksUpCurrentStateSoRewindIsRecoverable` proves the backup ref captures the pre-rewind tree; `AppChromeTest` and `InteractiveCommandTest` assert the banner; `NativeTerminalParityIT` tolerates both banners in the shared native+Java driver and verifies Ajent's banner directly instead of native parity for this line. The backup ref is internal (never in the checkpoint picker), so every other observable surface stays at parity. Preventing silent data loss on a destructive action outweighs banner parity. |
| Interactive unsandboxed warning | AgenTTY's interactive TUI runs process tools unsandboxed (auto mode with no OS backend, e.g. Windows/WSL without bubblewrap) with no in-UI indication. | Ajent prints the sandbox status to stderr at startup (matching its own ACP/MCP servers) and shows a persistent `⚠  sandbox: unavailable, running unsandboxed …` row in the status chrome whenever process tools are not OS-sandboxed. | `ProcessSandboxTest` covers the new `sandboxed()` signal across off/auto/on and every backend state; `InteractiveCommandTest.unsandboxedNoticeShowsPersistentWarningInChrome` proves the persistent chrome row appears only when unsandboxed. Surfacing an unsandboxed bash environment is a security-honesty improvement that adds only UI chrome and a startup line — no wire, persistence, or tool behavior changes. |
| File-state cache bound | AgenTTY keeps its process-wide read/snapshot file-state cache unbounded for the whole process lifetime. | Ajent bounds each cache (`READ_CACHE`, `SNAPSHOTS`) to the 8192 least-recently-used entries so long-lived ACP/MCP server processes cannot grow without limit. Realistic sessions stay far below the cap and never evict. | `FileToolsTest.fileStateCacheEvictsLeastRecentlyUsedBeyondCapacity` proves LRU eviction and that eviction only drops the `STALE_READ` re-read optimization or a stale-edit *warning* prefix; it never blocks a read/write or alters file bytes. The cache is an in-memory optimization with no protocol-visible surface, so bounding it changes nothing observable in any realistic session while removing an unbounded-growth leak in server modes. |

## Feature ledger

| Surface | Reference fixture | Java test | Implementation | Differential result | Status |
|---|---|---|---|---|---|
| CLI and configuration | help/version/invalid raw fixtures plus the pinned Windows executable, `src/runtime/main.cpp` dispatch, `skills.cpp::cmd_skills`, and `airgap/airgap.cpp` | `AjentCliTest`, `AuthCommandsTest`, `SkillCommandsTest`, `McpServeCommandTest`, `AcpCommandTest`, `AirgapCommandTest`, `ProcessSandboxTest`, `CliParityTest`, `NativeExecutableParityIT`, `InteractiveCommandTest` | exact parser/help/version/invalid handling, including native Windows CRLF process output and the terminal blank usage line; testable top-level command seam; working API-key/OAuth `login`, secret-safe `status`, idempotent `logout`, six-root `skills` inventory with source/path/description/resources/model-visibility, full spec warnings, empty-install guidance, and CI exit status; operational `mcp-serve` and `acp` with native workspace validation, protocol isolation, and sandbox lifecycle; ACP additionally applies settings/provider credential precedence, Ask-default profile validation, and keyless-local prompt authorization; operational `airgap` with reverse dynamic SOCKS SSH, native liveness ordering and exit propagation, terminal markers, explicit and synthesized X11/Wayland clipboard relay, fail-fast three-step credential setup, complete trust-boundary help, and no-PTY Zed ACP configuration generation; interactive startup restores provider/model/effort/profile and durable always-allow grants, with profile changes atomically clearing the trust baseline | default full reactor and compiled Java 25 ACP stdio launch green; opt-in executable gate verifies the binary hash and exact process exit/stdout/stderr for version, help, invalid argument, isolated status/logout, installed skills, air-gap help, and invalid-workspace ACP/MCP after only declared program-name/home normalization; generated Zed arguments also match the provided binary | complete |
| Pure FSM/model/edit/scheduler contracts | pinned C++ tests | translated unit tests, including all 13 scheduler cases | implemented | all 53 pinned native test programs are source-reviewed and mapped to green Java counterparts; downloaded-binary characterization supersedes an unsupported local Windows CTest build | complete |
...
therefore executable-proven.

The executable flow now chooses `a attach to composer` on a real PowerShell
Run Result, compares the resulting compact Output chip and status banner
cell-for-cell, and removes the entire chip with one Backspace. It also holds a
loopback response open, submits a second turn while streaming, and compares the
full queued user-turn rails, `queued #1 / 1` metadata, contextual recall/queue
placeholder, queued-count footer, and animated Braille status glyph. The
provider then completes and both clients automatically drain the pending turn.
Attachment attach/remove and live queue rendering/drain are executable-proven.

A separate loopback-provider case drives three complete turns through both real
executables, snapshots the terminal emulator's physical scrollback after every
turn, and requires every snapshot to retain the preceding snapshot as an exact
prefix. It then compares the final committed conversation cell-for-cell. This
also locks AgenTTY's settled-prose wrapping rule: use the full message body,
...
short-terminal `rows - 8` viewport clamp are shared production behavior.
Provider-picker rendering and navigation are executable-proven.
...
complete; checkpoint rendering and rewind are covered below.

Ctrl+J now opens the exact saved-thread picker in both executable clients. The
characterization seeds an older persisted conversation in both isolated homes,
then compares the active/current marker, title, local `MMM dd HH:mm` timestamp,
scrollbar rail, position counter, navigation hints, blank padding, and border
cell-for-cell. It moves Down and proves that only the selected row retains the
selection rail, presses Enter, waits for the asynchronous load, and compares
the rehydrated user/assistant conversation cell-for-cell. The native 0.2.8
binary's home-plus-viewport-clear wire is pinned; Ajent retains the checked-in
source's newer viewport-plus-scrollback hard reset. The harness uses negotiated
CSI-u Ctrl+J and Escape sequences, so the lifecycle is tested under the same
enhanced keyboard mode as production. Saved-thread picker navigation and real
thread switching are executable-proven.

Ctrl+K `rewind` now opens the exact oldest-to-newest checkpoint picker in both
executables over isolated, seeded Git repositories. The characterization waits
for every asynchronous diff summary to resolve, then compares `#turn` previews,
relative times, file/add/delete facts, selected rail, warning border,
restoration explanation, key hints, padding, and scrollbar cell-for-cell.
Enter performs a real rewind in both clients; the executable gate proves the
later file is removed, the transcript is truncated, the original prompt is
refilled in the composer, the inline surface resets, and the native
`▎ ▶ rewound · files restored, prompt back in composer` banner is exact.
Checkpoint picker and rewind behavior are executable-proven.
...
supports native `c`/`o` and `Ctrl-Y`/`Ctrl-O` URL actions, persists encrypted API-key/OAuth credentials (including expiry), installs
...
The 80x24 executable differential now opens Login through the command palette
in both clients, compares the full method picker and Anthropic API-key panel
cell-for-cell, types an identical deterministic secret, and compares the
masked view without submitting or launching a real browser. Browser handoff,
URL clipboard/reopen actions, exchange, persistence, and failure recovery use
fake browser/HTTP boundaries in the live tests so characterization cannot
touch a developer account.
