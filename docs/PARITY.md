...
| RAG and repository map | pinned `mcp-cpp` repository map plus AgenTTY `rag_test.cpp`, `rag_rerank_test.cpp`, `knowledge_test.cpp`, and advanced stemming/semantic/MMR/confidence/hot-reload/cache/MCP cases | `RepoMapToolsTest`, `RagCoreTest`, `RagRerankerTest`, `RagStemmerTest`, `RagCorpusFilesystemTest`, `KnowledgePipelineTest` | repository PageRank map; line-aligned semantic chunks and contextual BM25 with opt-in Porter normalization; RRF, reranking, extractive compression, MMR, structured contexts/confidence; document folder walks and v3 incremental cache; provenance-stamped corpus/MCP sources, lazy MCP refresh, cross-source deduplicating RRF router, metadata filters, query normalization, and composable retrieve/rerank/compress/MMR pipeline | deterministic lexical, filesystem, and knowledge-routing foundation ported; neural reranking, embeddings/HNSW, memory fusion, and differential capture remain | partial |
