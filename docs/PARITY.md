...
| Native tool catalog/dispatch | pinned `mcp-cpp` argument/filesystem/search/git/host/memory/web sources, web tests, and AgenTTY `toolset_e2e_test.cpp` | filesystem/search/process/git suites plus `HostToolsTest`, `MemoryToolsTest`, `WebToolsTest`, `ToolRuntimeTest`, edit matcher/repair tests | all 23 catalog bodies: safe filesystem/search/map tools, subprocesses/diagnostics/git, host and memory protocols, HTTPS-only fetch with HTML/link extraction and SSRF guards, and Brave/DuckDuckGo/Startpage fallback search | every catalog body has a Java protocol implementation; dispatcher wiring, JDK web adapter, persistent memory/RAG/skill backends, idle-timeout/output-spill parity, richer grep context, cache parity, and native differential capture remain | partial |
...
| Skills and memory | pinned `mcp-cpp` `host_tools.cpp`, `memory.cpp`, and AgenTTY toolset e2e cases | `HostToolsTest`, `MemoryToolsTest` | typed injected host contracts, exact todo rendering, skill/task refusal paths, document hit formatting, memory scope/tag/supersede/dedup/forget/wipe UX | protocol shells ported; skill discovery and durable memory store remain | partial |
| RAG and repository map | pinned `mcp-cpp` `repomap.cpp` and `search_tools_test.cpp` | `RepoMapToolsTest` | bounded source scan, definition/reference graph, 24-iteration weighted PageRank, focus personalization, greedy byte budget | repository map ported; persistent RAG indexing/retrieval and native differential capture remain | partial |
