...
method-not-found handling. A loopback OpenAI-compatible provider also drives
...
continuation, and final stop reason. This differential exposed and now
regresses native omission of empty pending `rawInput`, tool-metadata-before-
usage ordering, and structured file-change projection. Cancellation,
permission rejection/allow-always, concurrency, authenticated startup, and
client filesystem/terminal callback differentials remain open.
