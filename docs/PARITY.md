...
continuation, and reaches the same final response. The gate also compares both
complete upstream OpenAI-compatible request bodies after workspace-only
normalization, including
system/user/history messages, the provider-specific 22-tool subset and order,
...
canonical tool-argument serialization. Cancellation, permission allow-always,
concurrency, authenticated startup, and
