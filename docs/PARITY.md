...
| Agent loop/reducer | `domain/session.hpp` phase/retry proofs, `provider/error_class.hpp`, `tool/util/partial_json.cpp`, `runtime/app/update` stream/tool lifecycle, scheduler, doom-loop, and salvage-dedup sources | `SessionPhaseTest`, `ActiveTurnTest`, `ProviderErrorPolicyTest`, `PartialJsonTest`, `AgentReducerTest`, `AgentLoopTest`, `DispatcherToolPortTest`, `ToolSchedulerTest`, `DoomLoopBreakerTest`, `SalvagedCallDeduplicatorTest` | sealed four-phase typestate with active context absent from idle and exact transition legality; pure immutable submit/stream/tool/permission/cancel reducer with stale-turn rejection, UTF-8 8 MiB caps, typed effects, catalog-backed tool adapter, path-aware parallel scheduling, and a virtual-thread effect interpreter reusable by CLI/protocol/TUI/subagents; exact typed HTTP and wire-error classification, six-step jittered backoff ladders, Retry-After clamping, 90-second budget decay, monotonic four-attempt mid-stream protection, heartbeat/first-delta health resets, scheduled-retry deduplication/cancellation, and FIFO queued-turn draining; native partial-JSON closing and required-field guards, two-attempt same-context upstream-truncation recovery, retained block-close truncation signals, and non-retryable `max_tokens` protection; loop breaker and immutable re-leak reducer | foundational headless turns, tool continuations, transient/rate-limit recovery, and truncated tool-input recovery execute end-to-end; auth refresh, stall watchdog emission, compaction, all 15 message domains, persistence integration, and canonical differential traces remain | partial |
