...
| RAG and repository map | pinned `mcp-cpp` repository map plus AgenTTY `rag_test.cpp` and semantic chunking cases | `RepoMapToolsTest`, `RagCoreTest` | repository PageRank map; line-aligned semantic document chunks, fenced-code/list preservation, overlap-safe breadcrumb context, contextual BM25, cosine, reciprocal-rank fusion, BM25 corpus and multi-query fusion | deterministic BM25 foundation ported; stemming, rerank/compression/MMR, filesystem hot reload/cache, embeddings/HNSW, knowledge routing, memory fusion, and differential capture remain | partial |
