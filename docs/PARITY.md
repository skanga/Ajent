...
| Native tool catalog/dispatch | pinned `mcp-cpp` `arg_reader_test.cpp`, `fs_tools_test.cpp`, tool sources, and AgenTTY `toolset_e2e_test.cpp` | `ArgReaderTest`, `WorkspaceSandboxTest`, `FileToolsTest`, `ToolRuntimeTest`, edit matcher/repair tests | shared alias/coercion layer, symlink-safe workspace/read-root boundary, typed result/errors/FileChange, durable `read`/`write`/`edit`/`list_dir` dispatch with stale-read cache, ranges, binary/size guards and fuzzy edits | pinned filesystem assertions translated; remaining search/process/git/web/host tools and native differential capture remain | partial |
