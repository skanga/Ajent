...
| Native tool catalog/dispatch | pinned `mcp-cpp` `arg_reader_test.cpp`, `fs_tools_test.cpp`, `search_tools_test.cpp`, tool sources, and AgenTTY `toolset_e2e_test.cpp` | `ArgReaderTest`, `WorkspaceSandboxTest`, `FileToolsTest`, `SearchToolsTest`, `RepoMapToolsTest`, `ProcessToolsTest`, `BashValidatorTest`, `ToolRuntimeTest`, edit matcher/repair tests | shared alias/coercion layer, symlink-safe workspace/read-root boundary, typed result/errors/FileChange, durable filesystem dispatch, filename globbing, paginated regex grep, definition search, repository maps, bounded JDK subprocess execution, unattended-command validation, and build diagnostics | pinned filesystem/search/bash/diagnostics assertions translated; remaining git/web/host tools, idle-timeout and output-spill parity, richer grep context, cache parity, and native differential capture remain | partial |
