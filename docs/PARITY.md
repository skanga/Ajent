...
| Native tool catalog/dispatch | pinned `mcp-cpp` `arg_reader_test.cpp`, `fs_tools_test.cpp`, `search_tools_test.cpp`, `git.cpp`, tool sources, and AgenTTY `toolset_e2e_test.cpp` | `ArgReaderTest`, `WorkspaceSandboxTest`, `FileToolsTest`, `SearchToolsTest`, `RepoMapToolsTest`, `ProcessToolsTest`, `BashValidatorTest`, `GitToolsTest`, `GitToolsClassificationTest`, `ToolRuntimeTest`, edit matcher/repair tests | shared alias/coercion layer, symlink-safe workspace/read-root boundary, typed result/errors/FileChange, filesystem/search/map tools, bounded JDK subprocesses, unattended-command validation, diagnostics, and repository-aware status/diff/log/commit using shell-safe argv | pinned filesystem/search/bash/diagnostics/git assertions translated; remaining web/host tools, idle-timeout and output-spill parity, richer grep context, cache parity, and native differential capture remain | partial |
