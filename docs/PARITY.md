...
| RAG and repository map | pinned `mcp-cpp` repository map plus AgenTTY `rag_test.cpp`, `rag_expand_test.cpp`, `rag_rerank_test.cpp`, `knowledge_test.cpp`, and advanced stemming/semantic/MMR/confidence/hot-reload/cache/MCP/neural cases | `RepoMapToolsTest`, `RagCoreTest`, `RagQueryExpanderTest`, `RagRerankerTest`, `NeuralRerankerTest`, `RagStemmerTest`, `RagCorpusFilesystemTest`, `KnowledgePipelineTest`, `AgenttyDocRetrieverTest` | repository PageRank map; line-aligned semantic chunks and contextual BM25 with opt-in Porter normalization; RRF and opt-in generative multi-query expansion; deterministic feature reranking plus bounded-wave opt-in neural scoring, both through concrete JDK Ollama adapters; extractive compression, MMR, contexts/confidence; v3 folder cache; provenance-stamped multi-source router, filters, normalization, composable pipeline, and canonical `search_docs` funnel | lexical, filesystem, expansion, source-fusion, knowledge-routing, and optional neural `search_docs` stages ported; embeddings/HNSW, interactive composition, and differential capture remain | partial |
