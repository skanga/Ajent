...
| Agent loop/reducer | `domain/session.hpp` phase/retry proofs, `runtime/app/update` stream/tool lifecycle, scheduler, doom-loop, and salvage-dedup sources | `SessionPhaseTest`, `ActiveTurnTest`, `AgentReducerTest`, `AgentLoopTest`, `DispatcherToolPortTest`, `ToolSchedulerTest`, `DoomLoopBreakerTest`, `SalvagedCallDeduplicatorTest` | sealed four-phase typestate with active context absent from idle and exact transition legality; pure immutable submit/stream/tool/permission/cancel reducer with stale-turn rejection, UTF-8 8 MiB caps, typed effects, catalog-backed tool adapter, path-aware parallel scheduling, and a virtual-thread effect interpreter reusable by CLI/protocol/TUI/subagents; loop breaker and immutable re-leak reducer | foundational headless turns and tool continuations execute end-to-end; transient/truncation retries, compaction, queued-turn draining, all 15 message domains, persistence integration, and canonical differential traces remain | partial |
