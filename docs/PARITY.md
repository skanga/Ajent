...
| RAG and repository map | pinned `mcp-cpp` repository map plus AgenTTY `rag_test.cpp`, `rag_rerank_test.cpp`, `knowledge_test.cpp`, and advanced stemming/semantic/MMR/confidence/hot-reload/cache/MCP cases | `RepoMapToolsTest`, `RagCoreTest`, `RagRerankerTest`, `RagStemmerTest`, `RagCorpusFilesystemTest`, `KnowledgePipelineTest`, `AgenttyDocRetrieverTest` | repository PageRank map; line-aligned semantic chunks and contextual BM25 with opt-in Porter normalization; RRF, reranking, extractive compression, MMR, structured contexts/confidence; document folder walks and v3 incremental cache; provenance-stamped docs/skills/memory/MCP sources, lazy refresh/reindex, cross-source deduplicating RRF router, metadata filters, query normalization, composable pipeline, and concrete canonical wide-rerank-MMR-compress `search_docs` backend | deterministic lexical, filesystem, source-fusion, knowledge-routing, and non-neural `search_docs` funnel ported; generative query expansion, neural reranking, embeddings/HNSW, interactive composition, and differential capture remain | partial |
