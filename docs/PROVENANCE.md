...
| Tool effects, dispatcher argument/error/output boundaries, and path-aware scheduling | `include/agentty/tool/effects.hpp`, `include/agentty/tool/spec.hpp`, `include/agentty/tool/tool.hpp`, `src/runtime/app/cmd_factory.cpp`, `tests/scheduler_path_test.cpp` |
...
| RAG chunking, BM25/dense fusion, HNSW graph serialization, query fusion, reranking/compression, knowledge routing, advanced stemming/MMR/confidence/hot reload/cache/MCP/neural behavior | `tests/rag_test.cpp`, `tests/rag_hnsw_test.cpp`, `tests/rag_expand_test.cpp`, `tests/rag_rerank_test.cpp`, `tests/knowledge_test.cpp`, `tests/rag_advanced_test.cpp` and their referenced `include/agentty/rag` implementation |
...
| CommonMark/GFM terminal block rendering, native table-shape classification, and structure-preserving live Markdown reveal | `tests/table_render_test.cpp`, `maya/include/maya/widget/markdown.hpp`, `maya/src/widget/markdown/streaming`, `src/runtime/view/thread/turn/turn.cpp` |
