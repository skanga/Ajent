...
public final class UnicodeWidth {
  ...
  public static int of(int codePoint) { return of(codePoint, Mode.MODERN); }

  public static int of(int codePoint, Mode mode) {
    ...
    if (inRanges(codePoint)) return 2;
    if (mode == Mode.MODERN && codePoint >= 0x1f1e6 && codePoint <= 0x1f1ff) return 2;
    return 1;
  }

  public static int stringWidth(String value, Mode mode) {
    java.util.Objects.requireNonNull(value, "value");
    java.util.Objects.requireNonNull(mode, "mode");
    return value.codePoints().map(codePoint -> of(codePoint, mode)).sum();
  }

  private static boolean inRanges(int codePoint) {
    int low = 0;
    int high = WIDE.length / 2;
    while (low < high) {
      int middle = (low + high) >>> 1;
      if (WIDE[middle * 2] <= codePoint) low = middle + 1;
      else high = middle;
    }
    return low > 0 && codePoint <= WIDE[(low - 1) * 2 + 1];
  }
}
